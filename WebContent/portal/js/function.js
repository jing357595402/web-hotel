...
function SetCookie(name, value, expires) {// 写cookie
	...
	document.cookie = name + "=" + escape(value) + ";expires="
			+ exp.toGMTString();// +";domain=arkoo.com;path=/";
}
	...
	// Initialize for IE/Windows ActiveX version
		...
		try {
			...
			try {
				...
				newsstring = "<div class='loading'>Loading rquest content fail, Please try it again latter...</div>";
			}
		}
	}
}
...
function getnews(tagid, x) {
	...
	if (ajccache[url] == null) {
		var loadstatustext = "<div class='loading'><img src='portal/imgs/loading.gif' /> Loading request content, please wait...</div>";
		rqtp[tagid] = tagid;
...
function aet(tagid, etp) {
	...
	if (readcookie(tagid + "_cntd") == "none") {
		...
		if (parseInt(readcookie(wnf)) > 1)
			...
			TabNews(tagid, 1);
		if (etp == 1)
			aw = tagid + readcookie(wnf);
	}
	for (var iy = 0; iy < lis.length; iy++) {
		with (lis[iy]) {
			if (etp == 0) {
				lis[iy].onmouseover = function() {
					rT = attributes.getNamedItem('id').value.substring(0,
							attributes.getNamedItem('id').value.length - 1);
					if (getObject(rT + '_cnt').style.display != "none"
							&& className != "")
						return;
					idn = parseInt(attributes.getNamedItem('id').value
							.substring(
									attributes.getNamedItem('id').value.length - 1,
									attributes.getNamedItem('id').value.length));
					clearTimeout(waitInterval);
					waitInterval = window
							.setTimeout(
									"etype=0;TabNews(rT,idn);if(getObject(rT+'_cnt').style.display == 'none')displayList(rT+'_cnt'); SetCookie(rT+'info',idn,10000000);SetCookie(rT+'_cntd','ture',10000000);",
									MouseDelayTime);
				}
				lis[iy].onmouseout = function() {
					if (getObject(rT + '_cnt').style.display != "none"
							&& className != "")
						return;
					clearTimeout(waitInterval);
			...
			if (etp == 1) {
				...
				lis[iy].onclick = function() {
					...
					rT = attributes.getNamedItem('id').value.substring(0,
							attributes.getNamedItem('id').value.length - 1);
					if (getObject(rT + '_cnt').style.display != "none"
							&& (idn == 1 && (this.className == "tabactive1") || aw == attributes
...
function hideText() {
	if (document.getElementsByTagName("arkoo:title").length == 0) {
		...
		getObject("closeit").innerHTML = '<a href="http://cn.arkoo.com/diaryfiles/showpage.aspx?username='
				+ upu
				+ '&articleid='
				+ upa
				+ '" target="_blank">查看原文</a> <a href="tab.html" target="_self">×</a>';
	}
}
...
function showClassList(oid) {
	...
	if (objid.style.display == "none") {
		if (objid.getElementsByTagName("img").length == 0)
			TabNews(oid.substring(0, oid.indexOf("_")), 1);
		...
		SetCookie(oid + 'd', "none", 10000000);
		SetCookie(oid + 'h', maxh[oid], 10000000);
	}
}